...
Welcome! You are an AI coding agent working in this repository. To ensure high-quality, stable, and reliable contributions, you must strictly follow the protocols defined below.
...
1. **Check Environment Health**: Run `./init.sh` (or `.\init.ps1` on Windows PowerShell) to ensure the project installs cleanly and the baseline checks/tests pass. If the initialization script fails, you must stop and fix the environment before proceeding with new features.
...
Before concluding your session or handoff, you must:
