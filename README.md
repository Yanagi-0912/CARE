...
長輩看診時錄音，轉成文字與摘要給家人看。入口在掛號提醒 T-1h 的「我已出發」卡片上（只有本人版），直接開到 LIFF 的 `/clinic-visits/record`。

- **一定要先徵詢醫師同意**。衛福部《醫療機構醫療隱私維護規範》第二點要求診療過程錄音須先徵得對方同意，所以第一屏是徵詢畫面而不是設定；`consent` 是必填、沒有預設值。醫師不同意時走 `self_recap`（出診間後自己複述），功能還活著，但畫面會標明那是就診者記得的版本。
- **系統不會說哪一句是醫師講的**。轉錄開了語者分離，但只拿來決定哪裡換段，段落不帶講者身分。官方寫「3 位以上的講者歸屬是實驗性質」，而診間本來就有三到五個人；標錯「講者 2」會誤導讀的人，只做段落分隔則最多多一個換行。摘要的提示詞也明著禁止寫「醫師說」。
- **藥名靠使用者自己的用藥清單標示，不改寫逐字稿**。`custom_vocabulary` 不能和語者分離並用（官方限制），所以放棄熱詞。比對母體是這位使用者當日仍有效的用藥，不是全庫——全庫模糊比對換掉一個劑量數字約 2% 必然釘錯。門檻 0.80 由 `scripts/measure_clinic_drug_hints.py` 量出，完整結果在 `drug_hints.MATCH_THRESHOLD` 的註解。劑量數字一律不比對也不更正。
- **音檔不存**。寫進暫存檔 → 背景轉錄讀它 → 不論成功失敗都刪掉。資料庫裡只有文字，保存 30 天由 Mongo TTL 執行，與對話原文同一條線。
- **轉錄在背景跑**。上傳立刻回一筆 `status=processing`，十分鐘的門診錄音撐不過一個 HTTP 請求。失敗標 `failed` 且不重試（音檔已刪）。
- 權限走 `SENSITIVE`，不是 `GENERAL`：診間對話至少和用藥適應症一樣敏感。

| 設定 | 說明 | 預設 |
| --- | --- | --- |
| `CLINIC_RECORDING_MAX_BYTES` | 上傳上限。真正的限制是時間（開語者分離後單檔 30 分鐘），這個值只擋明顯誤用 | 40 MB |

- **逐字稿統一轉成台灣正體**。轉錄模型指定了 `zh-TW`，華語仍回簡體、台語卻回正體，所以整份先判斷再用 OpenCC `s2tw` 轉（只換字形不換用語）；正體再轉一次會把「干擾」變「幹擾」。

2026-09-22 用 30 分鐘 podcast 實測（整檔送）：華語 96 秒、台語 110 秒。台語品質不能用——整檔送輸出被截斷（`MAX_TOKENS`，最後 5 分鐘沒轉），切段送幾乎全部丟掉。

還沒驗證：LIFF 內建瀏覽器能不能用麥克風（尤其 iOS，只能用真手機驗）、實際診間的收音品質。
