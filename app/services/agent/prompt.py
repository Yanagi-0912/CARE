...
def build_system_prompt(language: str) -> str:
    ...
    return (
        ...
        "問本人時 `person` 與 `relationship` 留空；明確姓名填 `person`；家人稱謂填 "
        "`relationship`（parent／child／spouse／sibling／grandparent／grandchild）；"
        "同一句同時明確提供姓名與稱謂時兩者都填，讓系統檢查是否一致。"
        "只有本輪明確屬於該看診者的年齡與性別才填 `age`、`gender`（male／female），"
        "禁止從兒子、女兒、老婆等稱謂推測年齡或性別，也禁止套用發話者的資料；"
        "懷孕、生產、月經或生殖情境必須保留在 `symptom` 原文中；使用者明確點名科別時，"
        "另將部定專科名稱填入 `requested_department`，未點名則留空；"
        ...
        "   - 「我5歲兒子嘔吐要看哪科」→ `symptom=嘔吐`、`relationship=child`、`age=5`；"
        "「我老婆肚子痛要看哪科」→ `symptom=肚子痛`、`relationship=spouse`，不得填發話者的"
        "年齡或性別；「王美玲頭痛要看哪科」→ `person=王美玲`；"
        "「我父親王大明頭痛要看哪科」→ `person=王大明`、`relationship=parent`；"
        "「我懷孕了而且肚子痛要看哪科」→ `symptom=懷孕了而且肚子痛`；"
        "「我肚子痛可以看婦產科嗎」→ `symptom=肚子痛`、`requested_department=婦產科`。\n"
