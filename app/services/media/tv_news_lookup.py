...
class TvNewsArticleFinder:
    ...
    # 2026-09-18／19 實測（35 組真實螢幕標題 × 前五筆搜尋結果，逐筆人工判讀是不是
    ...
    #   0.25 以下開始收進接錯的
    #
    # **螢幕標題與網站標題常常是兩套寫法**，所以不能用高門檻。實際線上案例：
    # 「維他命添色素‧影響智力」的網站版是「維他命摻色素 專家：恐過敏傷智力」
    # （0.70）；「i-dle薇娟健康檢查片惹議 恐觸犯南韓醫療法」的三立網站版是
    # 「i-dle薇娟拍健檢片爆違法！舒眠胃鏡過程全公開遭韓政府單位報警處理」
    # （0.48，2026-09-19 就是差這 0.02 沒附到連結）。
    #
    # 真正接錯的（別集節目、標籤頁、同主題的另一篇）最高只有 0.25，0.25～0.48
    # 之間整個是空的，門檻放在這段空白中間。不再往下調到 0.30：那裡開始逼近
    # 接錯的那一群，多撈到的一則不值得。
