...
class DrugCatalogService:
    ...
    def _match_by_fuzzy(self, key: str) -> Optional[DrugCatalogMatch]:
        ...
        #   藥證，連錯得穩不穩定都談不上。
        #   逐行程的變異不只出在這裡的分數同分，上游的候選池本身就會變：
        #   `_candidates` 的 `min(non_empty, key=len)` 在多個 gram 的 postings
        #   長度並列時，選到哪一個取決於 `_ngrams` 回傳的 set 的迭代順序，
        #   聯集進去的就是不同的集合（實測抽樣 4,000 筆查詢有 824 筆存在這種
        #   並列，PYTHONHASHSEED 0／1／42／7 的候選池**成員**四次皆不同——池的
        #   大小恰好相同，所以只比大小會漏看）。
        #   但這個不確定性到不了任何真實藥名：全庫 56,886 個中文品名跑完
        #   `match()`，釘定證號與完整候選清單在 PYTHONHASHSEED 0／1／42／7
        #   下位元相同（pinned 皆為 19,204）。會受影響的只有字面上比不到任何
        #   鍵、因而落到這裡的查詢，而這條路徑本來就一律不釘證號——變動的是
        #   「這個錯字還救不救得回來」，不是「救回來的是哪一顆藥」。
        #   證號一釘就解析得出縮圖，推播與
