...
同樣 `AsyncIOMotorClient`、同樣 lazy `_ensure_collection()`），差別只在檢索
的 path 是 claim 向量欄位而非內容 embedding，以及輸出型別是單一
`ClaimMatch | None` 而非 `list[Document]`——這裡要的是「有沒有比對到」，
不是排序後的候選清單。
...
# 誤配是本設計唯一嚴重的失效模式，門檻寧缺勿濫）。這裡的預設值只是安全下限，
# 實際數字要等 tasks 1.2 用 TFC 自身 claim 做正負樣本校準後才能定案。
DEFAULT_CLAIM_MATCH_MIN_SCORE = 0.9
...
class MongoAtlasClaimMatcher:
    """正規化主張 → embedding → MongoDB `$vectorSearch`（claim 欄位）→ 是否已被查核過。"""
