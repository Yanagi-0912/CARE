...
- 同一位就診者、同一時間、同醫院、同科別已經有一筆（`cancelled` 除外）→ 409（§4）。
  - 醫院的比法：兩邊都有 `facility_id` 時只比 id，所以同名的連鎖分院不算重複；只要有一邊沒有 id，才改比院名。
  - 院名與科別都忽略空白。
  - 同一時間但不同醫院或不同科別**不擋**：同一家醫院上午兩個科同時報到是常見的。
...
| POST、PUT | 同一位就診者、同一時間、同醫院、同科別已經有一筆（比對規則見 §2.2） | `這個時間已經有一筆同醫院、同科別的掛號提醒，不需要重複建立。` |

PUT 只在 `appointment_at`、`facility_id`、`hospital_name`、`department` 其中之一有帶時才做重複檢查，比的是改完之後的樣子，並排除自己。只改備註的請求不會因為別筆而失敗。
...
| 20 | （未規定）重複的掛號 | 同一位就診者、同一時間、同醫院、同科別只能有一筆，第二筆回 409；同一時間不同醫院或不同科別不擋 | 重複的兩筆會各自推播，家屬收到兩倍的通知；其中一筆按了到診，另一筆仍會在 T+30 發出「尚未到診」。這是應用層檢查，兩個請求在同一瞬間送出同一筆時仍可能都通過 |
