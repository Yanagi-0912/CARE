...
這個落差反映到線上資料庫：目前 1,367 筆食藥署闢謠專區的 chunk（佔知識庫的 31.5%），
...
- 目前知識庫裡 **97% 的資料來自兩支會回傳結構化 JSON 的政府 API**（衛福部闢謠網站
  2,840 筆 + 食藥署闢謠專區 1,367 筆 = 4,207／4,339），這兩支本來就不需要「網頁爬取」，
  用 API 直接拿結構化資料反而比較穩定、比較不會因為網站改版而壞掉。
  （刪除 266 筆導覽列噪音之前這個比例是 91%，噪音清掉後 API 來源的佔比更高了。）
...
- 真正需要網頁爬取的只有台灣事實查核中心（TFC）這個來源，目前是 132 筆 chunk，
  只佔知識庫的 3.0%（132／4,339），用現有的 `scraper_tfc.py`（BeautifulSoup + 翻頁邏輯）
  處理量級上是夠的，沒有急迫性要換工具。
...
## 2026-08-09 追記：題庫稽核後的兩項修正與兩項新觀察

我們這邊對 RAG 評測題庫做了逐題稽核，過程中有幾個發現與這份報告直接相關，
其中一項是對第 5 項主張的**誠實修正**，照實補上。

### 修正：第 5 項「碎片拖累 reranker」在我們的資料上暫時量不到現象

先前的機制推測是「500 字元硬切的斷句碎片會讓 cross-encoder reranker 表現變差」。
兩個更新：

**文獻面（支持）**：2026 年多篇文章支持這個機制——
[vstash（arXiv 2604.15484）](https://arxiv.org/abs/2604.15484)實測 cross-encoder
rerank 無法在 RRF 融合基線上加分；[Towards Data Science 生產案例](https://towardsdatascience.com/your-chunks-failed-your-rag-in-production/)
實測固定切分 context recall 0.72、改句子視窗後 0.88；BigDataBoutique 指出答案句與
支撐語境被切進不同 chunk 時兩邊都拿不到高分。

**本地實測面（修正）**：我們把題庫標籤修正之後（詳見下一節），
**Cohere rerank 在現有的碎片語料上反而勝過 RRF 混合排序**（nDCG@5 +0.032）。
也就是說「碎片拖累 reranker」在 CARE 目前**沒有可觀察的現象可以歸因**——
先前量到的 rerank 劣勢是題庫標籤錯誤造成的假象。

**這不改變第 5 項的優先序**，但理由要修正為：
1. 檢索召回——仍有 3 題的正解卡在第 7–37 名進不了 top-5，兩種排序都救不動
2. 資料品質——127 筆 1 字元殘渣、480 筆 <100 字元碎片仍在
3. 標籤穩定性（見下一項新觀察）

### 新觀察一（已查明，結論修正）：標籤失配是我們自己的 bug，ETL 無涉

先前版本這一節把「7/34 題標籤比對不到 chunk」列為開放問題，並臆測了兩個機制
（上游改版重複入庫／我們的腳本正規化）。**已查明是後者，向你們致歉先前的臆測**：

- 字元級比對顯示，DB 內文與標籤的差異全部是**空白字元**
  （例：DB 現文 `8小時： 睡眠`、`。 ■ 飲食`，標籤裡沒有那些空白）
- 兇手是我們 CARE 端的標籤生成腳本 `rag_tighten_golden.py:102` ——
  `re.sub(r"\s+", "", text)` 在記錄標籤前把所有空白刪光，而評分是對原文精確比對。
  標籤出生時就是壞的，先前被 url 標籤扛著沒被發現。
- 也已確認**沒有**「同文兩版本並存」：抽查文章的 chunk_index 皆無重複、
  total_chunks 宣告一致。**此項對 ETL 端沒有任何行動需求。**

### 新觀察 7b（同一次查案發現的真問題）：部分寫入會留下永久破洞

查案時撞見一個實例：`pid=16703` 這篇文章 `total_chunks` 宣告 4，
但庫中 `chunk_index` 只有 `[1, 3, 4]` —— **第 2 塊不存在**。

機制（`main_pipeline.py:87-88`）：某個 chunk 的 `get_embedding()` 重試耗盡失敗時，
只印 `⚠️ 第 N 個切片失敗！` 然後跳過該塊、其餘照常寫入。之後這篇文章會被
`find_one` 判定「已存在」而永遠跳過 —— **破洞永遠不會補**。

**建議修改**：改成「全有或全無」——整篇文章所有 chunk 的向量都成功才 `insert_many`，
任一失敗就整篇不寫入、留給下次執行重試；或至少把失敗的 url 落地成清單供重跑。
順帶一提，這也讓第 7 項的停止條件更可靠（「已存在」才真的代表「完整存在」）。

### 新觀察二（非 bug，供設計參考）：語料充滿近重複文章

政府新聞稿的特性是同主題年年重發：庫裡「722 量血壓」至少 6 篇、「中風預防」7 篇、
「走路趣尋寶」4 篇。這對檢索排序與評測都有實質影響（我們在 CARE 端已用
多正解標籤與文章層級去重處理）。

對 ETL 端的意義：這給第 8 項「存 `發布日期`／`修改日期`」多了一個理由——
近重複文章群在下游需要「取最新」的依據，而現在整個 pipeline 把日期丟掉了。
