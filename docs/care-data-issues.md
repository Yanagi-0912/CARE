...
### 新觀察一：由庫內文字生成的標籤，8 天內腐化了 7/34——機制未明，開放問題

事實：我們的題庫在 2026-08-01 從當時 DB 的 chunk 內文複製了約 25 字的片段當標籤；
2026-08-09 檢查時，34 題中有 7 題的片段**已比對不到庫中任何 chunk**，
但那些文章的 chunk 都還在庫裡（各 2–6 個）。期間沒有人改過切片程式碼。

我們**沒有查明機制**，兩個候選解釋供你參考：

1. 上游文章改版後，若標題也變了，`upload_to_mongodb` 的
   `find_one({url 或 original_title})` 比對會失敗 → 同一篇文章的新版本被當新文章
   再入庫 → **同文兩版本並存**。可以查：同一 `url`（或近似標題）底下是否有
   內文不同的兩批 chunk。
2. 我們自己標籤生成腳本的文字正規化問題（我們這邊會再查）。

如果是機制 1，它會放大第 8 項（只 insert 不 update）的影響——不只更新不到，
還會產生重複內容。
