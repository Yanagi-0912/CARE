...
- **API**：新增 `POST /api/medications/prescription-scan`（multipart 影像上傳，回傳草稿）、`GET /api/medications/prescription-drafts/{draft_id}`、`POST /api/medications/prescription-drafts/{draft_id}/commit`；既有的 `GET /api/medications/reminders` 回應新增 `medications` 欄位（已解析的藥品，省去前端逐一查詢每個 id）。**路由前綴是 `/api/medications`**——router 定義在 `app/routers/users/medications.py`，但 `app/main.py:102` 掛載於 `/api/medications`，不是 `/api/users/medications`
...
- **相依**：`GEMINI_API_KEY` 已存在（`app/dependencies.py:71`），沿用既有 `GeminiService`，不新增外部服務。藥證庫為建置期產出的靜態檔，執行期不對外連線。新增套件 `python-multipart`（FastAPI 的 `UploadFile`／`File` 必要相依，專案先前沒有任何 multipart 端點所以未安裝）
