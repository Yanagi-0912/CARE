...
- [ ] 7.4 候選超過上限時先以顏色／形狀收窄再呈現照片；收窄後仍過多則不顯示照片、退回純文字（實測：多候選佔 37.8%，中位數 3 張、80% ≤5 張，但 5.3% 超過 21 張、最大 74 張）
- [ ] 7.5 藥品清單與提醒卡片依證號呈現照片與外觀描述；無照片時仍呈現文字描述
- [ ] 7.6 藥名被編輯時證號與照片一併失效（既有行為，補測試釘住）
- [ ] 7.7 文案全走 i18n，六語系同步
- [ ] 7.8 `CARE-LIFF/src/tests/prescriptionScan.test.tsx` 補 7.2～7.6
