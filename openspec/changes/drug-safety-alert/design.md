...
- **誤報疲勞讓功能自我廢除** → 這是本能力最可能的失敗方式。家人收到幾次無意義的通報後就會忽略通知，長輩發現「我一問就有人被叫來」之後就不再問，改去問朋友。三級分流、決策 4 第三列判 `low`、決策 5 的合法調劑辨識，三者都是為了壓低誤報。`SAFETY_ALERT_ENABLED` 預設開啟（產品決定：不希望每個環境都要各設一次），但開關本身保留為煞車——誤報率一旦失控，設成 `false` 即可整條停用，不必等一次 deploy。代價是誤報率的觀察變成上線後的事，而不是上線前的閘門。
