...
- [x] 6.1 非處方藥加入提醒後觸發偵測；處方藥完全略過
- [x] 6.2 重複時：當事人本人 + 收件人都通知；無重複時：只通知收件人
- [x] 6.3 對主流程 fail-open——任一步失敗記 log 後靜默結束
- [x] 6.4 log 不得帶藥名、成分、姓名、機構（用藥組合即病史線索）
- [x] 6.5 測試涵蓋四種組合：處方藥／非處方藥無重複／非處方藥有重複／偵測拋例外

## 7. 訊息與卡片

- [x] 7.1 當事人的提示 SHALL 引導詢問藥師，SHALL NOT 給劑量建議或指示停藥
- [x] 7.2 家人的通知含藥名、用途、重複成分；語言與字級取**收件人本人**的設定（比照 `build_family_alert_flex`）
- [x] 7.3 用語以「讓家人幫你看一下」為框架，不是「已回報」——避免長輩覺得被監控而抗拒掃描
- [x] 7.4 純文字不得含 Markdown（`openspec/specs/line-reply-rules`）
- [x] 7.5 卡片大小走既有的 `size_guard`，超過上限退回純文字
