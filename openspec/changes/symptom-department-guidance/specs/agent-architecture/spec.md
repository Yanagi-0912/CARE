...
工具集 SHALL 於 `SYMPTOM_DEPARTMENT_ENABLED` 為 true 時包含 `suggest_department_for_symptom`，為 false 時 SHALL NOT 包含。該工具與 `get_rag_answer` 的分流 SHALL 由代理依問句形態自行選擇，系統 SHALL NOT 另設獨立的意圖分類步驟。
...
#### Scenario: 症狀科別建議工具依旗標納入

- **WHEN** `SYMPTOM_DEPARTMENT_ENABLED` 為 true
- **THEN** 工具集包含 `suggest_department_for_symptom`；為 false 時不包含，其餘工具不受影響
