...
- [x] 10.5c **家庭查詢辨識本人**：按姓名查稱謂時，以只投影 `name` 的查詢辨識登入者本人；僅完整姓名可命中，部分姓名不得推定本人，本人與家庭成員同名時須回報歧義；支援第一人稱別名與六語固定回覆（2026-09-21；家庭 service、Agent、repository 與六語範圍 1,390 項全綠）
...
- [x] 10.7 **解析失敗的可見回覆**：`ambiguous` 與姓名／關係衝突時反問且不查症狀表；`not_found` 仍可依本輪資料給一般建議，但不得讀任一家人 profile（2026-09-24；人物反問、六語目錄、家庭解析、Agent、科別卡片、依賴組裝與 LINE 回覆相關範圍 2,109 項全綠）
- [x] 10.8 **兒科改讀病人資料**：兒科判斷改讀 `PatientContext.age` 與「寶寶」提示；移除症狀路徑對發話者年齡 ContextVar 的依賴；子女稱謂不得推導年齡（2026-09-24；PatientContext、工具、Agent、六語、科別卡片、依賴組裝與 LINE handler 相關範圍 2,137 項全綠）
- [x] 10.9 **性別適用性**：男性本人一般腹痛抑制婦產科、男性替老婆詢問保留婦產科、性別未知不猜測；懷孕／生產／月經／生殖語意及明確指定科別高於 profile（2026-09-24；PatientContext、工具 schema、Agent、六語、科別卡片、依賴組裝與 LINE handler 相關範圍 2,161 項全綠）
- [x] 10.10 **跨輪人物延續**：只在同一就醫問題明確延續時解析「他／她」，新的第一人稱問題重設為本人；缺少唯一前文時反問，不永久保存推測人物（2026-09-24；Agent 跨輪人物契約、家庭解析、科別工具、六語與 LINE handler 相關範圍 2,162 項全綠）
- [x] 10.11 **同句多位看診者**：人物與各自症狀綁定；單一看診者工具須反問先處理哪一位或分別輸出，禁止共用一個全域 `current_patient`（2026-09-24；結構化 cases、Gemini schema 相容性、六語反問、Agent、家庭解析、科別工具與 LINE handler 相關範圍 2,186 項全綠）
- [x] 10.12 **科別整合驗收**：加入本人、配偶腹痛／生產、兒子年齡未知、同句多個年齡、姓名／關係衝突、未連結他人、無權限、跨輪代名詞與同句多人端到端測試（2026-09-25；真實人物解析、授權、科別服務、工具、Agent 與 LINE handler 相關範圍 2,196 項全綠）

### 10.C 緊急流程接入

- [x] 10.13 **急迫度的人物輸出**：急迫度結果攜帶一個或多個受影響人物、各自事件與 reporter 歸屬；既有 emergency／none 判定與紅卡觸發 SHALL 不變（2026-09-25；人物改在判定後另問一次 LLM、判定的 prompt 與 schema 逐字不動——併入同一次呼叫實測會把同句兩人急症判成不緊急；急迫度判斷器 62 項、資料集評估輸出與改動前逐字相同、全套 9,292 項通過，3 項既有失敗與本變更無關）
- [x] 10.14 **行動先到**：紅卡先送、不等待家庭查詢或人物反問；本人、已解析家人、歧義人物與未連結第三人使用正確或中性稱謂（2026-09-25；急迫度判定整個存入 Agent state 並帶回 handler；人物辨識、名單解析與家人通報一律排在紅卡送出之後；唯一解析的家人用使用者的稱呼、其餘一律「對方」，紅卡內文改為中性；Agent、handler、解析與六語相關範圍 3,505 項、全套 9,330 項通過，3 項既有失敗與本變更無關）
- [x] 10.15 **通知正確病人的照顧者**：自動通知改以解析後的 patient 為主體；有效家庭連結中的任一角色可回報，收件人仍依 patient 的 `emergency_detected` 政策選出，且回報不得授予 profile 讀權（2026-09-25；通知主體改由 patients_to_notify 選出：本人急症為發話者、家人須唯一解析且伺服器驗證對方族譜中有回報者，歧義、名單查不到、未連結第三人與查詢失敗一律不通知，認不出是誰（辨識失敗或模型回不明）當成發話者本人（2026-09-25 決定）；收件人照病人政策選並排除回報者，不走 SENSITIVE READ；代報時不轉原話、不送對病人本人說的「家人已知道」，回報者標示與結果文案留給 10.16／10.17；緊急通報、handler 與解析相關範圍 321 項、全套 9,360 項通過，3 項既有失敗與本變更無關）
- [x] 10.16 **正確標示回報者**：第三人原文標成 reporter 回報，不得顯示成 patient 本人發言；本人發話才可使用「剛才說」（2026-09-25；家人通報卡依回報者是否為病人切換：代報時開頭為「{回報者} 剛才在 CARE 回報 {病人} 的狀況」、原話標成「{回報者} 回報的內容」並保留、第一步改為先聯絡回報者或病人；回報者名字讀自回報者本人，缺名時用「一位家人」；六語文案與 LINE 驗證；通報卡、通報服務與 handler 相關範圍 1,132 項以上、全套 9,372 項通過，3 項既有失敗與本變更無關）
- [x] 10.17 **通知結果文案**：依人物種類與 `sent／no_recipient／disabled／failed` 選固定多語文案；任何未送達結果不得宣稱家人已收到（2026-09-25；notify 回傳四種結果，收件人判定失敗算 failed、不算 no_recipient；紅卡後給發話者一則訊息，依本人／唯一解析家人／其他人 × 結果選固定六語文案，家人另有「連結未確認、沒有自動通知」一句，其他人一律「對方」；取代原本的「家人已經知道」與 10.14 的稱謂提示，只有 sent 說已通知；全套 9,378 項通過，3 項既有失敗與本變更無關）
- [x] 10.18 **防止重複與濫用**：跨人物緊急回報須寫稽核紀錄，並對同一 reporter／patient／事件實作去重與頻率限制（2026-09-25；`emergency_reports` 每位急症者一筆、只可追加、60 天 TTL、不存原話；同一位病人 10 分鐘去重（借用 health_alert_claims，未送達即交還）；替別人回報限回報者 24 小時 5 次、病人 1 小時 3 次，只數送達，本人只去重；新增「剛才已通知過」「已多次通知」六語文案；全套 9,412 項通過，3 項既有失敗與本變更無關）
- [x] 10.19 **緊急整合驗收**：加入「我阿公跌倒」、兩位阿公歧義、路人昏倒、朋友自傷、過去已處理的跌倒、本人與阿公同句急症，以及各種通知結果的端到端測試（2026-09-25；`tests/unit/services/safety/test_emergency_pipeline.py` 從 LINE 文字訊息走真實 handler、Agent、急迫度判斷器、家庭名單解析、通報服務與家人卡，只替換 LLM、MongoDB 與 LINE API；另含本人急症、連結未確認、去重與限流，並逐一確認紅卡是第一則；驗收 17 項、全套 9,429 項通過，3 項既有失敗與本變更無關）

### 10.D 完成門檻

- [x] 10.20 全部人物案例對六種語言的可見文案完成；用藥、科別、急迫度、家人通知與 LINE handler 測試全綠；確認不存在發話者 age／gender 被誤套到其他人的平行路徑（2026-09-25；652 個文案鍵中，只有 9 個 admin／API 錯誤訊息刻意只有中英、科別卡外語刻意不帶症狀原名，人物相關文案六語齊全；刪除已無讀者的發話者年齡 ContextVar（handler、dispatcher 與 user_age），加守門測試防止再長回來；實測 Gemini 15 次未把發話者 profile 的年齡／性別填進家人的 cases；全套 9,425 項通過，3 項既有失敗與本變更無關）
