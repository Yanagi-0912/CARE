...
分類器可以是「本地模型先判、沒把握才問 LLM」的串接。本地模型的訓練資料 SHALL 涵蓋所有支援語言（`zh-TW`、`en`、`id`、`vi`、`th`、`ja`）：只以中文訓練時，外語的健康問題會被本地模型直接判為無關。

#### Scenario: 外語健康訊息不被本地模型擋下

- **WHEN** 使用者以英文傳送「I have a stomach ache and my knee is hurt.」
- **THEN** 本地模型不將它判為無關，`allow_rag` 不因本地判斷而為 `False`
