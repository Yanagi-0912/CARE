...
from app.services.rag.client.embedding_client import embed_document, embed_query
...
async def test_embed_query_success():
    ...
    with patch(
        "app.services.rag.client.embedding_client.httpx.AsyncClient",
        return_value=http_client_cm,
    ), patch("app.services.rag.client.embedding_client.settings") as mock_settings:
